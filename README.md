...
| 你想做什么 | 应该点击哪里 |
|---|---|
| 只想安装使用 | [Gitee 直接下载](https://gitee.com/legender134/desktop-companion/releases/download/v2.4.6/DesktopCompanion-2.4.6-setup.exe) / [GitHub 直接下载](https://github.com/Legender134/desktop-companion/releases/download/v2.4.6/DesktopCompanion-2.4.6-setup.exe) |
...
> 普通用户只需要下载其中一个平台的 `DesktopCompanion-2.4.6-setup.exe`。不要下载名字中带有 `Source`、`源代码` 的压缩包，它们是给开发者看的源码，不能直接安装。

### 三步开始使用

1. 从 Gitee 或 GitHub 下载并双击 `DesktopCompanion-2.4.6-setup.exe`。
...
[Gitee v2.4.6 发布页](https://gitee.com/legender134/desktop-companion/releases/tag/v2.4.6)和 [GitHub 最新版发布页](https://github.com/Legender134/desktop-companion/releases/latest)都提供可直接下载的安装程序：

| 文件 | 用途 | 普通用户需要吗 |
|---|---|---|
| `DesktopCompanion-2.4.6-setup.exe` | Windows 安装程序 | **需要，下载这个** |
| GitHub 自动生成的 `Source code` | 标签源码快照 | 不需要 |

本项目已经完成源码测试、PyInstaller 冻结、Inno Setup 当前用户安装、真实安装/升级/卸载冒烟测试和 Windows 桌面交互验收。v2.4.6 发布验证结果见 [docs/manual-qa-v2.4.6.md](docs/manual-qa-v2.4.6.md)，每个正式版本的主要变化见[更新日志](CHANGELOG.md)。

### 历史版本

新用户应优先安装 v2.4.6。旧版本仍保留用于学习、比较和回退：
