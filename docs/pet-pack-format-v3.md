...
- `maxDurationMs`：常驻动作的硬启动边界，范围 10000–600000 毫秒，且不能小于最短时间加爬升时间。当前动作结束后，如果下一常驻动作按其 `frameMs` 或 `frameDurations`、`repeatCount` 和 `holdMs` 计算会越过边界，程序会改为播放退出动作；退出动画可以在边界后自然播完。
