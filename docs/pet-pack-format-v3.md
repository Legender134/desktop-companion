...
- `frameCount`：普通动作 1–512 帧。`gaze` 是方向图集，仍只能使用 16、32 或 64 帧。
...
- `includeInShowcase`：是否被右键“动作展示”依次播放，默认为 `true`。设为 `false` 只会跳过整套展示，不会隐藏动作菜单，也不会单独关闭自主播放。
...
这样动作会出现在自己的右键菜单项中，但不会被“随机动作”“自主小动作”或通用“动作展示”选中。单动作512帧上限不能绕过整包限制：图集仍最多2048格、解码后最多5000万像素、WebP最大32 MiB、JSON最大64 KiB且动作最多64个。
...
- `maxDurationMs`：常驻动作的硬启动边界，范围 10000–600000 毫秒，且不能小于最短时间加爬升时间。当前动作结束后，如果下一常驻动作按其 `frameMs` 或 `frameDurations`、`repeatCount`、`holdMs` 及当前动画速度计算出的真实播放时间会越过边界，程序会改为播放退出动作；退出动画可以在边界后自然播完。
...
状态激活时会暂停普通注视、自动闲逛和其他自主动作。拖动、切换宠物或退出程序会立即清理状态；从右键菜单手动结束时，会先让当前状态内动作自然收尾，再播放退出动作。状态入口不会加入“动作展示”，避免一次展示被长状态占住。
